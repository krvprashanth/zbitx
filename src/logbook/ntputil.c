...
// This is a simple NTP client implimentation for the sBitx that will check the
// current computer time against the defined ntp server pool and adjust the
// local time on the machine accordingly if it is +/- 1 second out of sync.
// 6/30/24 W2JON
...
time_t time_sbitx() { return time(NULL); }

void rtc_write_ntp(int year, int month, int day, int hours, int minutes,
                   int seconds) {
  printf("Updating the RTC with network time\n");
  // Set the system time first, then sync to hardware clock
  // However, ntp_request already updates the system time if sync_sbitx_time is
  // called correctly? Actually, ntputil.c seems to just calculate the time. For
  // sBitx, we want to use 'hwclock -w' to write the system time to the RTC.
  system("hwclock -w");
}

void rtc_read() {
  // Rely on the kernel to have synchronized system time from RTC at boot.
  // If needed, we can force a sync.
  printf("Syncing system time from RTC\n");
  system("hwclock -s");
}
